...
expression_scores<-read.csv("/Users/harrietbenbow/Documents/Miguel/Hotspot/Wheat/FHB+DON/Expression_scores.csv", header=T)
...
for (d in diseases){
  ...
  ggsave(plot, file=paste(d, ".pdf"), width=300, height=300, unit="mm", dpi=400)
...
write.csv(all_stress_hotspots, file="all_stress_hotspots.csv", row.names=F)
...
for(i in unique(all_stress_together$Chromosome)){
  ...
  plot<-
    ...
    scale_color_manual( values=c("grey60", "orangered2")) +
    coord_cartesian(ylim=c(0,1))
  ggsave(plot, file=paste(i, "all_stress.pdf"), width=300, height=300, unit="mm", dpi=400)
}
