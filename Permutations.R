...
all <- read.csv("/home/hbenbow/miguel/wheat_all.csv", header=F)
...
# the gene column is 
expression_scores<-read.csv("/home/hbenbow/miguel/Expression_scores.csv", header=T)
...
write.csv(conseqs, file="consec.csv", row.names=F)
write.csv(densitys, file="density.csv", row.names=F)
