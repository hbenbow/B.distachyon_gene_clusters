...
setwd("/Users/harrietbenbow/Documents/Miguel/Hotspot/Wheat/FHB+DON/")
...
for (d in diseases){
  ...
  for(i in 1:nrow(df)){
    ...
    if(data$density >= 0.6){
      ...
      write.csv(density_together, paste("density", d, ".csv", sep=""))
    }
  }
  ...
  plot<-
    ggplot(df[!(df$Chromosome=="chrUn"),], aes(x=end, y=density)) +geom_jitter(size=1.7, aes(colour=Hotspot), alpha=0.6) +facet_wrap(~Chromosome, ncol=3) + xlab("Position (bp)") + 
    ylab("Gene Density") + ggtitle(paste(d)) + theme_bw() + 
    geom_hline(yintercept=0.6, alpha=0.7) +
    theme(text = element_text(size=18, colour="black")) + 
    coord_cartesian(ylim=c(0,1))+
    scale_color_manual( values=c("grey60", "orangered2"))
  write.csv(df, file=paste(d, "analysed.csv", sep="_"), row.names=F)
  ggsave(plot, file=paste(d, ".pdf"), width=350, height=250, unit="mm", dpi=400)
}
...
write.csv(all_stress_together, file="all_stress_together.csv", row.names=F)
...
for(i in unique(all_stress_together$Chromosome)){
  ...
  plot<-
    ...
    scale_color_manual( values=c("grey60", "orangered2")) +
    coord_cartesian(ylim=c(0,1))
  ggsave(plot, file=paste(i, "all_stress.pdf"), width=350, height=250, unit="mm", dpi=400)
}
